...
- Publish stores the room on this device and announces it to players already in the same Portals session.
- Copy map code encodes the complete bounded room, spawn, finish, traps, and runner appearance without requiring an outside server.
- Use map code validates and imports authored geometry without registering ordinary challenge codes in Trending.
- The Portals edition does not show map-link buttons, global visibility controls, or claim that device/session data is globally Trending. Previously shared links remain importable for backward compatibility.
...
- The optional standalone Next/Supabase edition has a real community-map API and browser. It is retained for a future web deployment but cannot be reached from a processed Portals game.
...
- Publishing writes the announcement to shared state and broadcasts it. Receiving players validate and register the explicit publication in their local Trending catalog without automatically replacing their active run.
- The feature is inert outside a processed Portals host, so localhost and downloaded bundles retain the same self-contained code behavior.
...
## Optional standalone backend path

The backend path is implemented by `supabase/migrations/0021_custom_map_publishing.sql`, `/api/maps/**`, and the `/maps` browser in the standalone Next edition. It becomes operational only when that separate deployment is connected to a Supabase project. Portals blocks all outside network access, so this backend is not a dependency or transport for the Portals release. With Supabase variables blank, `/maps` explicitly says the global backend is not connected and never presents IndexedDB data as global.
...
## Portals release test matrix

- Build, publish, update, and verify an old code still opens its immutable version.
- Paste a code as a different player/session and compare every piece, zone, endpoint, trap, and color.
...
- Exercise 1 byte below, exactly at, and 1 byte above code/message limits.
...
Current verification: unit tests cover malformed/oversized messages, late-join state, broadcast announcements, request/response, exact-room leaderboards, published-map popularity signals, and code-only Portals builder controls. An end-to-end repository test covers two different player databases, browser restart, two complete child rounds, old-code immutability, corruption, and the payload ceiling. Browser production smoke covers the separate Trending and Use map code actions, builder startup, opt-in guides, and the honest Portals SDK fallback. A two-browser SDK-host simulation covers publish to session state and late-join import into the published-map catalog.

The only remaining Portals release gate requires platform state that is not present in this workspace: upload the static bundle to a processed Portals preview and open two players in one real session. Supabase verification belongs only to the optional standalone edition and does not block the Portals release.

### Optional standalone Supabase release command
