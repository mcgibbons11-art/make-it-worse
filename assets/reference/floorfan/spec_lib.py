#!/usr/bin/env python3
"""ObjectSculptSpec authoring helpers for this asset.

Derived from `assets/reference/props/spec_lib.py`, which in turn generalised the
shape `assets/reference/toaster/author_toaster_spec.py` proved a spec has to have
to clear `validate_sculpt_spec.py --strict-quality`. Kept as a per-asset copy
rather than an import so this reference folder builds on its own, and so a change
made for one asset cannot silently alter another's spec.

Only the parts that do not vary per object live here - the quality contract, the
build passes, the self-correction loop, the component and material record shapes.
`author_<asset>_spec.py` supplies everything it measured off its own reference.

Nothing in here invents geometry. Every number an asset spec cites comes from its
...
def pbr_provenance(prop_id: str, material_id: str, confidence: float, note: str) -> dict:
    ...
    crop = HERE / "evidence" / "crops" / f"{material_id}-crop.png"
