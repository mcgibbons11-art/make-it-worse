...
SPEC = {
    ...
    "items": [
        "helmet", "crown", "cowboy", "beret", "earmuffs", "grin", "beard",
        "mask", "goggles", "aviator", "visorband",
    ],
    "fitBasis": "HEAD measurements and silhouette gates in createWardrobeModels.ts",
    "frontAxis": "+Z",
    "materialRoles": {
        "glass": "translucent visor and lens surfaces",
        "main": "player-selected shell, cloth, hair, and frame colour",
        "trim": "derived high-contrast edging and soft-goods detail",
        "steel": "hinges and hard frame hardware",
    },
    "visualReview": "artifacts/avatar-catalog-final-2026-07-31 head-hair and face-clothes front/side sheets",
    "reviewStatus": "fourth runtime-evidence refinement generated (grin inverted to tooth-band, mask reseated); reviewed against artifacts/face-shoe-review-2026-08-04",
}
...
function grin(root: THREE.Group) {
  // Fourth refinement, from the 2026-08-04 runtime evidence: the mouth-sized
  // ink cavity with a sliver of tooth photographed as a dark maw. The weights
  // are now inverted - a broad tooth band carries the whole shape, ink
  // survives as the outline seam, the smile parting, and two tooth gaps in
  // the upper row - and the corners lift so the read is cocky, not gaping.
  const outer = [[-0.096, 0.03], [-0.05, 0.042], [0.006, 0.046], [0.06, 0.04], [0.098, 0.028], [0.084, -0.014], [0.033, -0.034], [-0.009, -0.036], [-0.051, -0.028], [-0.082, -0.008]] as const;
  const band = [[-0.081, 0.022], [-0.043, 0.033], [0.005, 0.036], [0.052, 0.031], [0.083, 0.02], [0.07, -0.008], [0.028, -0.025], [-0.008, -0.027], [-0.043, -0.02], [-0.068, -0.003]] as const;
  const seam = mesh(frameShape(outer, band, 0.006, 0.006), "ink", "grin-outline-seam", [0, -0.166, 0.29]);
  const teeth = mesh(curvedShape(band, 0.008, 0.009), "cream", "grin-tooth-band", [0, -0.166, 0.292]);
  const parting = mesh(box(0.148, 0.005, 0.004, 0.002), "ink", "grin-smile-parting", [0.004, -0.168, 0.305]);
  parting.rotation.z = 0.06;
  add(root, seam, teeth, parting);
  for (const x of [-0.03, 0.024]) {
    const separator = mesh(box(0.004, 0.014, 0.003, 0.0015), "ink", `grin-tooth-gap-${x}`, [x, -0.159, 0.305]);
    separator.rotation.z = -0.05 - x * 0.4;
    add(root, separator);
  }
}
...
function mask(root: THREE.Group) {
  ...
  // Seated 0.012 higher and wrapped deeper after the 2026-08-04 evidence: at
  // -0.15 the top edge opened a shadow band under the eyes and the cover read
  // as a plate resting on the chin rather than cloth over it.
  const cover = mesh(curvedShape(outline, 0.012, 0.032), "main", "mask-soft-cover", [0, -0.138, 0.294]);
  add(root, cover);
