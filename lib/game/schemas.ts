...
// Composed segment recipes still have a real catalogue limit. Trap counts do
// not: authored rooms may begin deeper than an ordinary social chain, and the
// serialized payload byte guard is their trust boundary.
...
export const trapInstanceSchema = z.object({ id: z.string().min(3), type: trapTypeSchema, ownerUserId: z.string().nullable(), ownerName: z.string().min(2).max(24), ownerAvatarSeed: z.number().int(), depthAdded: z.number().int().positive(), zoneId: z.string(), position: vec3Schema, rotationY: finite, seed: z.number().int(), params: z.record(z.string(), z.union([z.number().refine(Number.isFinite), z.boolean(), z.string()])) }).strict();
...
export const challengeSchema = z.object({ id: z.string(), slug: z.string().regex(/^[a-z0-9-]{6,24}$/), chainId: z.string(), chainSlug: z.string(), parentSlug: z.string().nullable(), depth: z.number().int().nonnegative(), baseSeed: z.number().int(), levelVersion: z.literal(1), createdByName: z.string().min(2).max(24), createdByAvatarSeed: z.number().int(), addedTrap: trapInstanceSchema.nullable(), traps: z.array(trapInstanceSchema), ghostTrace: ghostTraceSchema.nullable(), stats: challengeStatsSchema, createdAt: z.iso.datetime(), isDemo: z.boolean(), track: trackSchema.optional() }).strict().superRefine((challenge, context) => { if (challenge.traps.length !== challenge.depth) context.addIssue({ code: "custom", message: "Trap snapshot must equal depth" }); });
